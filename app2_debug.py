...
if uploaded_file is not None:
    ...
    # -----------------------------
    Q95_THRESHOLD = 47.73
    st.info(f"'is_extreme' feature generated internally using threshold {Q95_THRESHOLD} mm")
    ...
    ax.plot(
        df_recent["rainfall_mm"].tail(30),
        label="Rainfall"
    )

    ax.plot(
        df_recent["relative_humidity_percent"].tail(30),
        label="Humidity"
    )
    ...
    if st.button("🔮 Generate Next-Day Forecast", type="primary"):
        ...
        # Rainfall Plot
        fig, ax = plt.subplots(figsize=(10, 4))
        ax.plot(df_recent["rainfall_mm"].values[-SEQ_LEN:], label="Observed (Last 30 Days)")
